...
and eval_cmd ai_opts st (cmd:command) = 
  ...
  | Score -> let (s1, s2) = st |> score in 
    let p1_c = get_p1_color st in 
    let p2_c = get_p2_color st in 
    ANSITerminal.(print_string [style_of_color p1_c] (string_of_color p1_c ^ "\'s "));
    print_string "Score: ";
    ANSITerminal.(print_string [style_of_color p1_c] ((string_of_int s1)^"\n"));
    ANSITerminal.(print_string [style_of_color p2_c] (string_of_color p2_c ^ "\'s "));
    print_string "Score: ";
    ANSITerminal.(print_string [style_of_color p2_c] ((string_of_int s2)^"\n"));
    play ai_opts st
