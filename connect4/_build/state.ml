...
let find_diagonals piece assoc n = 
  ...
  let p3 =  if check_pattern piece assoc n (-1,1) then [(-1,1)]
    else [] in 
  let p4 = if check_pattern piece assoc n (-1,-1) then [(-1,-1)]
    else [] in 
  let res = p1@p2@p3@p4 in
  if res = [] then [(0,0)]
  else res 

let rec diagonal_score_helper a board n direction = 
...
let score t = 
  ...
  let rec diagonal_score ass n : int = 
    if n = 1 then 0 
      ...
      match ass with 
      | a::b -> 
        let direction = find_diagonals a ass n in
        begin
          match direction with 
          | [] -> 0;
          | h::t' -> if h = (0,0) then
