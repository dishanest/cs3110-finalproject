...
System Testing (TODO)
  ...
  including exceptions. [print] and [print_win] were play-tested. We wrote a 
  helper function [apply_insertions] to simulate players making moves. 
...
(** [apply_insertions init_st insertions] is the state that results from applying 
    [insertions] to [init_state]. Switches turns to simulate a game. *)
let rec apply_insertions (insertions:(int*int) list) (init_st:State.t) = 
  match insertions with 
  | [] -> init_st
  | (c, v) :: t -> apply_insertions t (insert c v init_st |> tick_turn)
...
let diag = 
  (new_state (Red, Blue)  (6, 6) false) |> apply_insertions [
